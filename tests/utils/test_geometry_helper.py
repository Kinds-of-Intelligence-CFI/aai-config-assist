...
# Run in terminal to get per test breakdown: python -m unittest -v tests/utils/test_geometry_helper.py
...
class TestGeometryHelper(unittest.TestCase):
    def test_calculate_vertices_of_rotated_rectangle_when_rotation_is_0(self):
        ...
        result = calculate_vertices_of_rotated_rectangle(
            center=centroid, width=width, height=height, angle_deg=rotation
        )

        self.assertTrue(
            np.all(np.isclose(expected, result)),
            "The rotated lower_base_vertices are not as expected.",
        )

    def test_calculate_vertices_of_rotated_rectangle_when_rotation_is_positive_and_centroid_is_not_at_origin(
        self,
    ):
        x_centroid = 1
        y_centroid = -3
        ...
        sqrt_2 = np.sqrt(2)
        expected = np.array([[0, sqrt_2], [sqrt_2, 0], [0, -sqrt_2], [-sqrt_2, 0]])
    ...
    def test_calculate_vertices_of_rotated_rectangle_when_rotation_is_negative(self):
        ...
        expected = np.array([[-2, -1], [-2, 1], [2, 1], [2, -1]])
    ...
    def test_calculate_vertices_of_axis_aligned_rectangle(self):
        ...
        result = calculate_vertices_of_axis_aligned_rectangle(
            center=centroid, width=width, height=height
        )
    ...
    def test_calculate_clockwise_rotated_2d_point(self):
        ...
        result = calculate_clockwise_rotated_2d_points(
            points=original_points, angle_deg=angle
        )

        self.assertTrue(np.all(np.isclose(expected, result)))

    def test_determine_overlap_between_aligned_segments_spanning_positive_and_negative_subset_of_real_axis(
        self,
    ):
        seg1 = np.array([-1, 3])
        seg2 = np.array([2, 5])
    ...
    def test_determine_overlap_between_aligned_segments_in_negative_subset_of_real_axis(
        self,
    ):
        seg1 = np.array([-9, -4])
        seg2 = np.array([-7, -5])
    ...
    def test_get_min_max_projections(self):
        points = np.array([[0, 1], [0, 0.5], [1, 0], [-1, 0], [0, -0.5], [0, -1]])
        sqrt_2 = np.sqrt(2)
