...
class TestPhysicalItemHelper(unittest.TestCase):
    DEFAULT_ITEM_PARAMETERS_PATH = "src/definitions/item_default_parameters.yaml"

    def setUp(self):
        super().setUp()
        # Get the default item parameters
        with open(self.DEFAULT_ITEM_PARAMETERS_PATH, "r") as file:
            self.default_item_parameters = yaml.safe_load(file)

    def test_get_default_item_parameter(self):
        result = get_default_item_parameter(
            item_name="GoodGoal",
            param_name="colour",
            default_item_params=self.default_item_parameters,
        )
        expected = {"r": 0, "g": 256, "b": 0}
        assert (
            result == expected,
            "The expected default colour did not match the result",
        )

    def test_get_default_item_parameter_with_nonexistent_item_name(self):
        with self.assertRaises(KeyError):
            get_default_item_parameter(
                item_name="ThisIsNotAnItemName",
                param_name="colour",
                default_item_params=self.default_item_parameters,
            )
