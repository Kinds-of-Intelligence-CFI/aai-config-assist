...
def set_up_app_layout(
    fig_init: matplotlib.figure.Figure, aai_item_names: List[str]
) -> html.Div:
...
def _set_up_left_hand_section(
    fig_init: matplotlib.figure.Figure, style_guide: AppStyleGuide
) -> html.Div:
    layout = html.Div(
        [
            dcc.Graph(
                figure=fig_init, id="aai-diagram", style=style_guide.aai_figure_style()
            ),
        ],
...
def _set_up_right_hand_section(
    aai_item_names: List[str], style_guide: AppStyleGuide
) -> html.Div:
    layout = html.Div(
        [
...
def _set_up_config_params_layout(style_guide: AppStyleGuide) -> html.Div:
    layout = html.Div(
        [
            html.H2(
                CONFIG_PARAMS_SECTION_TITLE,
                id="heading-set-config-params",
                style=style_guide.heading_style(),
            ),
            dcc.Input(
                placeholder=PASS_MARK_TEXT,
                type="text",
                value="",
                id="pass-mark",
...
def _set_up_new_item_layout(
    ...
    layout = html.Div(
        [
            ...
            dcc.Dropdown(
                aai_item_names, id="item-dropdown", style=style_guide.dropdown_style()
            ),
            dcc.Input(
                placeholder=LENGTH_INPUT_TEXT,
                type="text",
                value="",
                id="spawn-x",
                style=style_guide.length_input_style(),
...
def _set_up_current_item_board_layout(style_guide: AppStyleGuide) -> html.Div:
    return html.Div(
        [
            html.Plaintext(
                id="output-current-item",
                children="No item selected",
                style=style_guide.normal_text_style(),
            ),
        ]
    )
    ...
    return html.Div(
        [
            dcc.Slider(
                ...
                marks=None,
                tooltip={
                    "placement": style_guide.tooltip_placement,
